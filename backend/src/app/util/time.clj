...
(defn plus
  ...
  (cond
    (instance? Temporal d)
    (.plus ^Temporal d ^TemporalAmount ta)

    (instance? Duration d)
    (.plus ^Duration d ^TemporalAmount ta)
...
(s/def ::cron-expr cron?)

(defn next-exec-time
  [^CronExpression crn ^Instant inst]
  (.toInstant (.getNextValidTimeAfter crn (Date/from inst))))

(defn prev-exec-time
  [^CronExpression crn ^Instant inst]
  (.toInstant (.getPrevFireTime crn (Date/from inst))))


(defn get-cron-offset
  [cron ref-t]
  (duration-between ref-t (next-exec-time cron ref-t)))

(defn get-next-inst
  [^CronExpression cron ^Duration offset]
  (let [now-t  (now)
        t2     (plus now-t offset)
        next-t (next-exec-time cron t2)
...
(defn offset-clock
  [base offset]
  (let [^Duration offset (duration offset)]
    (Clock/offset base offset)))
